...
namespace MvcTeam.Utilities.Services
{
    public static class PNumberTString
    {
        ...
        static string int2str(string snum)
        {
            ...
            else
            {
                ...
                if (L >= basex.Length)
                    throw new ArgumentOutOfRangeException(nameof(snum), "Number is too large to convert to words (maximum 15 digits).");
        ...
        public static string GetStr(string number)
        {
            if (number.StartsWith("-"))
                return "منفی " + GetStr(number.Substring(1));
    ...
    public static class AppConvertNumberToLetters
    {
        ...
        public static string ToLetters(object input)
        {
            ...
            //فرمت ثابت تا جداکننده اعشار در همه فرهنگ ها نقطه باشد
            var output = PNumberTString.GetStr(number.ToString(CultureInfo.InvariantCulture));
