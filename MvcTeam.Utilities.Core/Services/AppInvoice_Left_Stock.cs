...
namespace MvcTeam.Utilities.Services
{
    public class App_Invoice_Left_Stock
    {
        ...
        public void StartFunction(Guid id)
        {
            ...
            {
                throw new InvalidPluginExecutionException("خطا در دریافت رسید: " + ex.Message, ex);
            }
            ...
            {
                throw new InvalidPluginExecutionException("خطا در دریافت فروش متناظر: " + ex.Message, ex);
            }
            ...
            {
                throw new InvalidPluginExecutionException("خطا در دریافت رسیدهای پرداخت: " + ex.Message, ex);
            }
            ...
            {
                throw new InvalidPluginExecutionException("خطا در محاسبه موارد باقیمانده: " + ex.Message, ex);
            }
            ...
            try
            {
                foreach (var item in invoice.InvoiceItems)
                {
                    ...
                    //محصولات با شناسه محصول و واحد، و محصولات نوشتاری با توضیحات تطبیق داده میشوند
                    var regardingComputedItem = computedInvoiceItems.FirstOrDefault(x => (x.ProductId == item.ProductId && x.UomId == item.UomId) || x.Description == item.Description);
            ...
            {
                throw new InvalidPluginExecutionException("خطا در چسباندن موارد به این رسید: " + ex.Message, ex);
            }
