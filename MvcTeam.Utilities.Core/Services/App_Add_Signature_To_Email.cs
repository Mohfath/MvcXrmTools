...
namespace MvcTeam.Utilities.Services
{
    public class App_Add_Signature_To_Email
    {
        ...
        public string StartFunction(Guid id, Guid userId)
        {
            ...
            var matchingSignatures = signatures.Where(x => x.SignatureType == signatureType).ToList();

            if (matchingSignatures.Count > 1)
            {
                _tracingService.Trace("More Than One Signature Was Found");
                throw new Exception("More than one signature matches type " + signatureType);
            }

            var selectedSignature = matchingSignatures.SingleOrDefault();

            if (selectedSignature == null)
            {
                //اگر امضایی انتخاب نشده باشد متن ایمیل بدون تغییر برمیگردد
                if (signatureType == EmailSignatureTypes.Null)
                    return email.Body;
