...
public class Email_EmailTeam : WorkFlowActivityBase
{
    public Email_EmailTeam() : base(typeof(Email_EmailTeam)) { }
    ...
    private static List<Entity> ProcessUsers(IOrganizationService service, EntityCollection teamMembers, List<Entity> toList)
    {
        foreach (Entity e in teamMembers.Entities)
        {
            ...
            if (EmailRecipients.Contains(toList, e.GetAttributeValue<Guid>("systemuserid"))) continue;
